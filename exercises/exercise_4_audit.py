...
async def audit_once(state, entry: AuditEntry) -> None:
    """Write an audit entry only if the same semantic event was not logged already.

    LangGraph restarts a node from the top when resuming after `interrupt()`.
    The pre-interrupt audit rows in the HITL nodes therefore need a tiny
    idempotency guard so a resume does not create duplicate "waiting" events.
    """
...
async def node_human_approval(state):
    ...
    await audit_once(state, AuditEntry(
        agent_id=AGENT_ID,
    ...
    await audit(state, AuditEntry(
        ...
        reason=resp.get("feedback") or f"Reviewer chose {resp.get('choice', 'pending')}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=(
            ...
            if state.get("human_choice") or state.get("escalation_answers")
            else None
        ),
        decision=decision,
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"final_action=auto_{action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    return {"final_action": "auto_approved" if action == "committed" else action}
...
async def node_escalate(state):
    ...
    await audit_once(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason=f"Awaiting answers to {len(questions)} question(s)",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason=f"Received answers to {len(answers)} question(s)",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
